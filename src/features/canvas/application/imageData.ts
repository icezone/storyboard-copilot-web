// Web version: no Tauri dependencies
// isTauri() always returns false in the browser; Tauri-specific code paths are removed.
...
export function resolveImageDisplayUrl(imageUrl: string): string {
  // Web version: no Tauri file URL conversion; local paths are not supported in web.
  return imageUrl;
}

export async function persistImageLocally(source: string): Promise<string> {
  // Web version: no local file persistence; return source as-is.
  return source;
}
...
export async function prepareNodeImageFromFile(
  ...
  // Web version: read file as data URL and prepare via browser canvas.
  ...
  console.info(
    `[upload-perf][imageData] prepareNodeImageFromFile web-mode name="${file.name}" size=${file.size}B readDataUrl=${dataUrlElapsed}ms total=${Math.round(performance.now() - started)}ms`
  );
  return prepared;
}
...
export async function prepareNodeImage(
  ...
  const started = performance.now();
  // Web version: use browser canvas path only (no Tauri).
  try {
    ...
    console.info(
      `[upload-perf][imageData] prepareNodeImage web-mode total=${Math.round(performance.now() - started)}ms`
    );
    return {
